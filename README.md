...
- **NDVI / พื้นที่สีเขียว** — รายปี + 12 เดือน · ระดับจังหวัด และอำเภอ · ป่าทึบ (dense forest) + พื้นที่สีเขียวต่อคน (m²/คน) เทียบ**ค่าอ้างอิง** WHO 9 m² — ค่านี้นับพืชพรรณทุกชนิดจาก NDVI ไม่ใช่สวนสาธารณะที่เข้าถึงได้ตามนิยาม WHO จึงไม่ใช้ตัดสินผ่าน/ไม่ผ่าน (ดู `green-area-frontend/src/utils/greenMetric.js`)
- **คุณภาพข้อมูล NDVI** — ทุกค่า NDVI บอกด้วยว่า median composite ปีนั้นทำจากภาพ Sentinel-2 กี่ภาพ · ปลอดเมฆเฉลี่ยกี่ภาพต่อ pixel · ความไม่แน่นอนของค่ากลางรายปี (±NDVI ที่ 2σ) เทียบเกณฑ์ **GCOS-245** · และมีภาพครบทั้ง 3 ฤดูตามนิยาม **กรมอุตุนิยมวิทยา** หรือไม่ — แสดงทั้งบนหน้าจอ รายงาน PDF และ CSV
- **เรือนยอดไม้ 30% (กฎ 3-30-300)** — % เรือนยอดปกคลุมต่อจังหวัด/อำเภอ เทียบเกณฑ์ 30% ของ Konijnendijk (2023) พร้อมระยะห่างจากเกณฑ์เป็นจุด% และ km² · นับเฉพาะไม้ยืนต้นจาก **ESA WorldCover** (คนละนิยามกับ "พื้นที่สีเขียว" ที่มาจาก NDVI จึงอ่านคู่กับค่าอ้างอิง WHO ได้) + แนวโน้มรายปีจาก **Dynamic World**
...
- **วาดพื้นที่เอง (custom area)** — วาด polygon บนแผนที่แล้ววิเคราะห์ NDVI/ประชากร/m²ต่อคน/LST/AI Recommend เฉพาะพื้นที่นั้น · บันทึกไว้ในบัญชีได้
- **รายงาน PDF** — สรุปผลวิเคราะห์ + บริบทการใช้ที่ดิน export เป็น PDF/CSV
- **บัญชีผู้ใช้** — สมัคร (ใช้งานได้ทันที ไม่ต้องยืนยันอีเมล) / ล็อกอิน / ลืมรหัสผ่าน (Supabase Auth) · โปรไฟล์ (ชื่อ, หน่วยงาน) · ลบบัญชีถาวร · role `admin` สำหรับล้าง cache
- **ความยินยอมคุกกี้ (PDPA)** — แถบขอความยินยอม + ตั้งค่ารายหมวด · ระบบไม่ได้ตั้งคุกกี้จริง ที่เก็บทั้งหมดเป็น localStorage และไม่มีตัวติดตาม/analytics · หมวด "เพื่อการทำงาน" ต้องยินยอมก่อนจึงจะบันทึก และถอนแล้วลบทิ้งทันที · ข้อกำหนด/นโยบายความเป็นส่วนตัว/นโยบายคุกกี้ อ่านได้ในแอป
...
- เปิด SQL Editor รัน migration ตามลำดับ (ดู [green-area-backend/migrations/](green-area-backend/migrations/)):
  ...
  - `014_ndvi_data_quality.sql` (คอลัมน์ `data_quality` jsonb — คุณภาพ/ความไม่แน่นอนของ NDVI composite)
  - `015_canopy_cover.sql` (คอลัมน์ `canopy` jsonb — ตัวชี้วัดเรือนยอด 30% ตามกฎ 3-30-300)
- **ตั้งค่า Supabase Auth (จำเป็นสำหรับระบบล็อกอิน)** — ในหน้า Dashboard:
  1. **Authentication → Providers → Email** — **ปิด** "Confirm email" · แอปไม่มีหน้ายืนยันอีเมล สมัครแล้วเข้าใช้งานได้ทันที ถ้าเปิดไว้ผู้ใช้จะล็อกอินไม่ได้และเจอข้อความ "บัญชีนี้ยังไม่ได้ยืนยันอีเมล"
  2. **Authentication → URL Configuration → Site URL / Redirect URLs** — ใส่ URL ของแอป (`http://localhost:3000` สำหรับ dev) ให้ลิงก์รีเซ็ตรหัสผ่านเด้งกลับมาถูก · อีเมลฉบับเดียวที่ระบบส่งคือลิงก์รีเซ็ตรหัสผ่าน
