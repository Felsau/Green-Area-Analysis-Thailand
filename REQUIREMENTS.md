...
| FR-05 | จัดอันดับจังหวัดตามค่าพื้นที่สีเขียวต่อคน (urban subset — เขต built-up) เทียบ**ค่าอ้างอิง** WHO 9 m²/คน · ไม่ตัดสินผ่าน/ไม่ผ่าน เพราะระบบวัดพืชพรรณทุกชนิด ไม่ใช่พื้นที่สาธารณะที่เข้าถึงได้ตามนิยาม WHO | `[R1]` |
...
| FR-08 | แนะนำพันธุ์ไม้พื้นถิ่นตาม 6 ภูมิภาค พร้อมเหตุผลทางนิเวศ (21 ชนิด) | `[R5]` |
...
| NFR-08 | Accuracy / Validation | **ตรวจสอบความถูกต้อง** ของ green area เทียบ ESA WorldCover และรายงานค่าความคลาดเคลื่อน (เป้า ±10%) **(มีแล้ว — ผลอยู่ใน §5.1: MAE 5.53 จุด% · 60/77 จังหวัดอยู่ในเกณฑ์)** | `[R13]` |
...
### 5.1 ผลการตรวจสอบความถูกต้อง (NFR-08) — วัดจริง 77 จังหวัด

รันด้วย `validate_green_area.py` เมื่อ 2026-07-28 · ปี 2021 (epoch ของ WorldCover เพื่อตัด
ตัวแปรการเปลี่ยนแปลงพื้นที่ระหว่างปี) · scale 500 ม. · เทียบ `green_area_pct` (NDVI > 0.3)
กับคลาสพืชพรรณของ ESA WorldCover `[R13]` (10, 20, 30, 40, 90, 95)

| ตัวชี้วัด | ค่า |
|---|---|
| Mean error | **−1.33 จุด%** (NDVI ต่ำกว่าเล็กน้อย) |
| MAE | **5.53 จุด%** |
| RMSE | **7.42 จุด%** |
| อยู่ในเกณฑ์ ±10 จุด% | **60/77 จังหวัด (78%)** |
| คลาดเคลื่อนสูงสุด | นนทบุรี +19.0 จุด% |

**ข้อค้นพบหลัก: 17 จังหวัดที่เกินเกณฑ์อธิบายได้ครบด้วยกลไกเพียง 2 แบบ ที่มีทิศทางตรงข้ามกัน**

1. **นาข้าว (Cropland) → NDVI ต่ำกว่า** — 11 จังหวัด ทั้งหมดอยู่ในเขตที่ราบภาคกลาง/อีสาน
   (นครสวรรค์ −18.9 · สุรินทร์ −16.3 · บุรีรัมย์ −15.1 · อำนาจเจริญ −14.1 · ร้อยเอ็ด −14.0 ·
   มหาสารคาม −13.8 · นครราชสีมา −11.8 · พิจิตร −11.7 · ยโสธร −11.3 · ลพบุรี −11.0 ·
   กำแพงเพชร −10.9) · WorldCover จำแนกนาเป็น Cropland ตลอดทั้งปี แต่ median NDVI รายปี
   ของนาที่มีช่วงพักดิน/น้ำท่วมขังตกต่ำกว่า 0.3 → เฉลี่ยทั่วประเทศคลาสนี้ทำให้ต่างกัน 6.1 จุด%
2. **สิ่งปลูกสร้าง (Built-up) → NDVI สูงกว่า** — 6 จังหวัด ทั้งหมดเป็นเขตเมือง
   (นนทบุรี +19.0 · กรุงเทพฯ +16.9 · ปทุมธานี +12.2 · นครปฐม +11.8 · ภูเก็ต +11.7 ·
   สมุทรปราการ +10.8) · NDVI จับหญ้า/ต้นไม้แทรกอาคารได้ แต่ WorldCover ตัดสินทั้ง pixel
   เป็นสิ่งปลูกสร้าง (กลไกเดียวกับที่ทำให้ DW ต่ำเกินจริงในเมือง) → เฉลี่ย 2.7 จุด%

กลไกทั้งสองหักล้างกันในระดับประเทศ mean error จึงเหลือเพียง −1.33 จุด% · **ความต่างที่พบ
ไม่ใช่ความผิดพลาดของการวัด แต่เป็นผลจากนิยาม "พื้นที่สีเขียว" ที่ต่างกัน** — NDVI วัด
*พืชพรรณที่เขียวจริงในช่วงเวลานั้น* ส่วน WorldCover วัด *ประเภทการใช้ที่ดิน* ซึ่งยืนยันว่า
การใช้ NDVI คิดค่า m²/คน (เทียบค่าอ้างอิง WHO) และใช้ WorldCover ตอบเกณฑ์เรือนยอด 30% ของ FR-17
เป็นการเลือกที่ถูกต้อง ไม่ควรยุบเหลือชุดข้อมูลเดียว

**การกระทบยอด** ทุกจังหวัดตรวจย้อนได้ครบด้วย `error_pp = net_pp + reference_scale_delta_pp`
(ค่าคลาดเคลื่อนคงเหลือสูงสุด 0.10 จุด% = การปัดเศษ) โดย `reference_scale_delta_pp` คือส่วนต่าง
ของค่าอ้างอิง WorldCover ระหว่างการอ่านแบบ fractional (10 ม.) กับแบบ plain ที่ scale วิเคราะห์

**ผลพลอยได้ — ปริมาณของ pyramid MODE** `reference_scale_delta_pp` วัดผลของปัญหา MODE ที่
อธิบายไว้ในหัวข้อก่อนหน้าเป็นตัวเลข: เฉลี่ยทั้งประเทศ +0.59 จุด% แต่ช่วงกว้าง −8.5 ถึง +3.1
จุด% โดยติดลบมากที่สุดในเขตเมืองที่พืชพรรณกระจัดกระจาย — เป็นหลักฐานเชิงปริมาณสนับสนุน
การเลือกใช้ `_fractional_area` ใน FR-17

**ข้อจำกัดของวิธี** `_fractional_area` (ที่ต้อง `reproject` ไป 10 ม.) ใช้กับ mask ที่พึ่ง
composite ของ Sentinel-2 ไม่ได้ — GEE ตอบ `User memory limit exceeded` ทุกจังหวัดแม้ band
เดียวและ `tileScale=16` · พจน์ที่ตัดกับ `green_mask` จึงวัดแบบ plain ที่ scale วิเคราะห์
(ซึ่งเป็นวิธีเดียวกับที่ระบบคิด `green_area_pct` อยู่แล้ว จึงกระทบยอดกันได้พอดี)
...
ก่อนหน้า (2026-07-07) เลื่อน `[RW1]` → `[R32]` (Moukomla et al. 2026, Earth 7(3):76) เป็น "ใช้แล้ว" หลัง implement ปัจจัย peri-urban ใน `scoring.py` — รวมอ้างอิงที่ใช้จริงเป็น 32 รายการ · ก่อนหน้า (2026-07-06) เพิ่ม `[R27]`–`[R31]` (แหล่งข้อมูล/แพลตฟอร์ม) · ตอนนั้นใช้คู่กับ Proposal (`presentation/generate_proposal_pdf.py` ข้อ 7.4.3–7.4.4 — โฟลเดอร์นี้ถูกลบออกจาก repo ภายหลัง 2026-07-28)_
